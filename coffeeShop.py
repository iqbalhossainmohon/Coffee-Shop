...
class LoginWindow:
    def __init__(self, root):
        self.root = root
        self.root.title("Coffee Shop Management System - Login")
        self.root.geometry("400x300")
        self.root.resizable(True, True)
        self.root.configure(bg="#6F4E37")
    ...
    def create_auth_tables(self):
        ...
        if self.cursor.fetchone()[0] == 0:
            ...
            self.cursor.execute(
                ...
                ("admin", hashed_password, "admin", created_at)
            )
            self.conn.commit()
    ...
    def create_login_ui(self):
        ...
        # Main frame
        main_frame = tk.Frame(self.root, bg="#6F4E37")
        ...
        # Login frame
        login_frame = tk.Frame(main_frame, bg="#F5F5DC", bd=2, relief=tk.GROOVE)
        ...
        # Username
        username_frame = tk.Frame(login_frame, bg="#F5F5DC")
        ...
        self.password_entry = ttk.Entry(password_frame, show="*", font=('Helvetica', 10))
    ...
    def authenticate(self):
        ...
        if user:
            ...
            self.root.destroy()
            root = tk.Tk()
            app = CoffeeShopManagementSystem(root, user)
            root.mainloop()
        else:
...
class CoffeeShopManagementSystem:
    def __init__(self, root, user):
        self.root = root
        self.user = user  # Logged in user information
        ...
        user_frame = tk.Frame(self.header_frame, bg=self.main_color)
        user_frame.pack(side=tk.RIGHT, padx=10)
        
        user_label = tk.Label(
            user_frame, 
            text=f"Welcome, {self.user[1]} ({self.user[2]})", 
            font=('Helvetica', 10), 
            bg=self.main_color, 
            fg='white'
        )
    ...
    def add_sample_data(self):
        ...
        if self.cursor.fetchone()[0] == 0:
            # Add sample products
            sample_products = [
                ("Espresso", "Coffee", 3.50, 1.00, 100),
                ("Cappuccino", "Coffee", 4.00, 1.20, 100),
                ("Latte", "Coffee", 4.50, 1.50, 100),
                ("Mocha", "Coffee", 5.00, 1.80, 100),
                ("Americano", "Coffee", 3.00, 0.80, 100),
                ("Green Tea", "Tea", 2.50, 0.70, 100),
                ("Black Tea", "Tea", 2.00, 0.60, 100),
                ("Croissant", "Pastry", 2.50, 0.90, 50),
                ("Muffin", "Pastry", 2.00, 0.70, 50),
                ("Sandwich", "Food", 5.50, 2.00, 30)
            ]
        ...
        if self.cursor.fetchone()[0] == 0:
            ...
            sample_customers = [
                ("John Doe", "555-0101", "john@example.com", 0),
                ("Jane Smith", "555-0102", "jane@example.com", 0),
                ("Bob Johnson", "555-0103", "bob@example.com", 0),
                ("Alice Brown", "555-0104", "alice@example.com", 0)
            ]
    ...
    def create_nav_buttons(self):
        ...
        # Add user management button for admin users only
    ...
    def generate_receipt(self, order_id, customer_id, order_date, total):
        ...
        # Receipt content
        receipt_frame = tk.Frame(recept_window)
    ...
    def show_user_management(self):
        """Show user management content (admin only)"""
        if self.user[2] != "admin":
        ...
        for text, field in fields:
            ...
            if field == "user_password":
                ...
                entry = ttk.Combobox(frame, values=["admin", "staff"], state="readonly")
                entry.set("staff")
            else:
                entry = ttk.Entry(frame)
    ...
    def populate_users(self):
        """Populate users in the treeview"""
    ...
    def clear_user_form(self):
        """Clear the user form"""
        for field, entry in self.user_entries.items():
            if field == "user_role":
                entry.set("staff")
            else:
                entry.delete(0, tk.END)
    ...
    def add_user(self):
        """Add a new user"""
        username = self.user_entries["user_username"].get().strip()
        password = self.user_entries["user_password"].get()
        role = self.user_entries["user_role"].get()
        ...
        try:
            query = '''
                ...
                VALUES (?, ?, ?, ?)
            '''
            self.cursor.execute(query, (username, hashed_password, role, created_at))
            self.conn.commit()
